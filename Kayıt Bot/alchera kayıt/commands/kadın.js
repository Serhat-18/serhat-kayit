...
exports.run = async (client, message, args) => {
    ...
    try {
        ...
        if (!piece) return message.channel.send(new Discord.MessageEmbed().setDescription(`Bir kullanıcı belirtmelisin. **Örnek: @Serhat**`).setFooter(config.footer).setColor("#10181046").setTimestamp());
        if (!Serhat) return message.channel.send(new Discord.MessageEmbed().setDescription(`İsim belirtmelisin. **Örnek: Serhat**`).setFooter(config.footer).setColor("#10181046").setTimestamp());
        if (!alchera) return message.channel.send(new Discord.MessageEmbed().setDescription(`Yaş belirtmelisin. **Örnek: 18**`).setFooter(config.footer).setColor("#10181046").setTimestamp());

        message.channel.send(new Discord.MessageEmbed().setDescription(`**__Kayıt İşlemi Başarılı__**\n\n Kayıt Edilen Kişi: ${piece}\n Kayıt Yapan Yetkili: ${message.author}\n Kayıt İşleminde Verilen Rol: <@&${config.kadın}>\n Kayıt İşleminde Alınan Rol: <@&${config.kayitsiz}>`).setFooter(config.footer).setColor("#7c00ff").setThumbnail(config.gif).setTimestamp())
        piece.setNickname(`${config.tag} ${Serhat} | ${alchera}`).catch(e => message.channel.send(`Benden Üstte Olduğu İçin İsmini Değiştiremedim.`))
        await piece.roles.add(config.kadın) //$erhat/18
